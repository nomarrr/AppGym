...
@Component({
  selector: 'app-membership-all-payment-stats',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './membership-all-payment-stats.component.html',
  styleUrl: './membership-all-payment-stats.component.css'
})
export class MembershipAllPaymentStatsComponent implements OnInit, OnDestroy {
  chart: any;
  ...
  loadPaymentData() {
    this.membershipStatsService.getAllPaymentStats(this.selectedPeriod).subscribe({
      next: (data) => {
  ...
  createChart(data: { labels: string[], values: number[] }) {
    const ctx = document.getElementById('allPaymentChart') as HTMLCanvasElement;
    if (!ctx) return;
